// Panel flotante de resultados. Shadow DOM para aislarse del CSS de la página.
// Misma postura que la web: evidencia primero, veredicto a pedido, abstención digna.
...
(() => {
  ...
  function panel() {
    ...
    root.getElementById("x").addEventListener("click", () => { host.remove(); host = null; });
  ...
  function asked(claim) {
    return `<p class="asked"><span class="lbl">Afirmación consultada</span>${esc(claim)}</p>`;
  }
  ...
  function render(msg) {
    ...
    if (msg.type === "linterna:loading") {
      body.innerHTML = asked(msg.claim) + `<p class="muted">Buscando evidencia…</p>`;
      return;
    }
    if (msg.type === "linterna:error") {
      body.innerHTML = asked(msg.claim) +
        `<p class="muted">No se pudo verificar. ¿Está corriendo el servidor de Linterna?</p>`;
      return;
    }
  ...
  chrome.runtime.onMessage.addListener((msg) => {
    if (msg && typeof msg.type === "string" && msg.type.startsWith("linterna:")) render(msg);
  });
})();
